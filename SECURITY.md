...
`file-hash` runs entirely in your browser. There is no server component and no
account system, so the file you hash is never uploaded. Most classic web vulnerabilities
...
- The hashing logic (`src/utils/hashEngine.ts`), the service worker, and the PWA manifest.
- Anything that could send file data or metadata off the device.
