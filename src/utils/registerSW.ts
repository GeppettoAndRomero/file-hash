...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/file-hash/sw.js, scope=/file-hash/）。
    // scope がツールの全 locale ページ（/file-hash/, /file-hash/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/file-hash/sw.js', {
      scope: '/file-hash/',
    });
